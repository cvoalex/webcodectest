...
class DirectWebSocketServer:
    ...
    async def handle_single_request(self, request_data, start_time):
        ...
        # DIRECT CALL to inference engine - USE REAL AUDIO when provided!
        audio_to_use = None
        if audio_data and len(audio_data.strip()) > 0:
            # We have real audio data from microphone
            audio_to_use = audio_data
            print(f"🎤 Using real-time audio: {len(audio_data)} chars")
        else:
            # No audio provided, use cached model audio
            audio_to_use = None
            print(f"💾 Using cached model audio")
