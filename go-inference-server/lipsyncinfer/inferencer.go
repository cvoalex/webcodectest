...
// NewInferencer creates a new ONNX inferencer with custom library path and GPU ID
func NewInferencer(modelPath string, libraryPath string, gpuID int) (*Inferencer, error) {
	...
	// Append CUDA provider with specific GPU
	cudaOptions, err := ort.NewCUDAProviderOptions()
	if err == nil {
		defer cudaOptions.Destroy()
		err = cudaOptions.Update(map[string]string{
			"device_id": fmt.Sprintf("%d", gpuID),
		})
		if err == nil {
			err = sessionOptions.AppendExecutionProviderCUDA(cudaOptions)
			if err != nil {
				fmt.Printf("⚠️  Warning: Could not enable CUDA on GPU %d: %v\n", gpuID, err)
			} else {
				fmt.Printf("✅ CUDA enabled on GPU %d\n", gpuID)
			}
		}
	}
...
// InferBatch runs batch inference sequentially
...
// GetGPUID returns the GPU ID this inferencer is using
func (inf *Inferencer) GetGPUID() int {
	return inf.gpuID
}
