...
try:
    ...
    # Add specific path for data_utils.ave to avoid conflicts
    ave_path = os.path.join(parent_dir, 'data_utils', 'ave')
    if ave_path not in sys.path:
        sys.path.insert(0, ave_path)
    
    from data_utils.ave.test_w2l_audio import AudDataset
...
class MultiModelInferenceEngine:
    ...
    async def generate_inference_only(self, model_name: str, frame_id: int, audio_override: Optional[str] = None) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
        """Generate only the inference result + bounds for client-side compositing (base64 audio)"""
        ...
        try:
            ...
            # Return inference result + bounds + metadata
            metadata = {
                "model": model_name,
                "frame_id": frame_id,
                "has_audio": bool(audio_override),
                "inference_time_ms": int(inference_time * 1000),
                "audio_shape": list(audio_to_use.shape) if hasattr(audio_to_use, 'shape') else None,
                "processing_time_ms": int((time.time() - start_time) * 1000)
            }
            ...
            print(f"❌ Inference generation failed for model '{model_name}' frame {frame_id}: {e}")
            metadata = {
                "model": model_name, 
                "frame_id": frame_id,
                "error": str(e),
    ...
    async def generate_inference_only_binary(self, model_name: str, frame_id: int, audio_binary: Optional[bytes] = None) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
        """🚀 OPTIMIZED: Generate inference result with raw binary audio (NO base64 conversion)"""
        ...
        try:
            # 🚀 OPTIMIZATION: Process binary audio directly (skip base64 conversion!)
            if audio_binary and len(audio_binary) > 0:
                # Calculate hash for caching
                audio_hash = self._calculate_audio_hash_binary(audio_binary)
                if audio_hash not in self.audio_cache:
                    print(f"🚀 OPTIMIZED: Processing {len(audio_binary)} raw bytes directly - NO BASE64!")
                    audio_features = await self._extract_audio_features_binary(audio_binary)
                    self.audio_cache[audio_hash] = audio_features
                else:
                    print(f"🚀 OPTIMIZED: Using cached binary audio features")
                audio_to_use = self.audio_cache[audio_hash]
            else:
                # Use cached audio from model
                audio_to_use = self.audio_cache[model_instance.audio_hash]
            ...
            metadata = {
                "model": model_name,
                "frame_id": frame_id,
                "has_audio": bool(audio_binary),
                "inference_time_ms": int(inference_time * 1000),
                "audio_shape": list(audio_to_use.shape) if hasattr(audio_to_use, 'shape') else None,
                "processing_time_ms": int((time.time() - start_time) * 1000),
                "optimization": "binary_direct"
            }
            
            return prediction, bounds, metadata
            
        except Exception as e:
            print(f"❌ Binary inference generation failed for model '{model_name}' frame {frame_id}: {e}")
            metadata = {
                "model": model_name, 
                "frame_id": frame_id,
                "error": str(e),
            ...
            raise RuntimeError(f"Binary inference failed: {e}") from e

    def _calculate_audio_hash_binary(self, audio_binary: bytes) -> str:
        """🚀 OPTIMIZED: Calculate hash for binary audio data"""
        import hashlib
        return hashlib.md5(audio_binary).hexdigest()
    
    async def _extract_audio_features_binary(self, audio_binary: bytes) -> torch.Tensor:
        """🚀 OPTIMIZED: Extract audio features from raw binary data (no base64 conversion)"""
        
        if not AUDIO_EXTRACTION_AVAILABLE:
            raise RuntimeError("Audio extraction not available")
        
        return await asyncio.get_event_loop().run_in_executor(
            self.executor, self._extract_audio_sync_binary, audio_binary
        )

    def _extract_audio_sync_binary(self, audio_binary: bytes) -> torch.Tensor:
        """🚀 OPTIMIZED: Synchronous audio extraction for raw binary data"""
        
        if audio_binary and len(audio_binary) > 0:
            from data_utils.ave import audio
            from data_utils.ave.hparams import hparams
            
            try:
                # Convert binary audio directly to numpy array (skip base64 decode!)
                audio_array = np.frombuffer(audio_binary, dtype=np.int16).astype(np.float32) / 32767.0
                
                print(f"🚀 OPTIMIZED Audio processing: {len(audio_array)} samples ({len(audio_array)/24000*1000:.1f}ms) - NO BASE64!")
                
                # Validate audio data
                if len(audio_array) == 0:
                    print("❌ Empty audio array!")
                    return torch.zeros((1, 32, 16), dtype=torch.float32)
                
                if np.isnan(audio_array).any() or np.isinf(audio_array).any():
                    print("❌ Invalid audio data (NaN or Inf)!")
                    return torch.zeros((1, 32, 16), dtype=torch.float32)
                
                # Expected: 15,360 samples for 640ms at 24kHz
                expected_samples = 16 * 960  # 16 chunks × 960 samples each
                if len(audio_array) != expected_samples:
                    print(f"⚠️ Warning: Expected {expected_samples} samples, got {len(audio_array)}")
                ...
                # Process 640ms of audio to generate mel spectrogram
                mel_spec = audio.melspectrogram(audio_array).T  # Shape should be [time_frames, 80]
                
                print(f"🎵 Mel spectrogram shape: {mel_spec.shape}")
                
                # Crop to 16 time frames (like crop_audio_window does)
                # Take the middle 16 frames from the mel spectrogram
                if mel_spec.shape[0] >= 16:
                    start_idx = (mel_spec.shape[0] - 16) // 2
                    end_idx = start_idx + 16
                    cropped_mel = mel_spec[start_idx:end_idx, :]  # [16, 80]
                else:
                    # Pad if we don't have enough frames
                    cropped_mel = np.pad(mel_spec, ((0, 16 - mel_spec.shape[0]), (0, 0)), 'constant')
                
                print(f"🎵 Cropped mel shape: {cropped_mel.shape}")
                
                # Convert to tensor and reshape to match model expectation [1, 32, 16]
                # Take first 32 mel bins and transpose
                if cropped_mel.shape[1] >= 32:
                    # Take first 32 mel bins and transpose
                    mel_reshaped = cropped_mel[:, :32].T  # [32, 16]
                    mel_tensor = torch.from_numpy(mel_reshaped).unsqueeze(0).float()  # [1, 32, 16]
                else:
                    # Fallback if we don't have enough mel bins
                    mel_tensor = torch.zeros((1, 32, 16), dtype=torch.float32)
                
                print(f"🎵 Final tensor shape: {mel_tensor.shape}")
                print(f"🚀 OPTIMIZED Tensor min: {mel_tensor.min()}, max: {mel_tensor.max()}, mean: {mel_tensor.mean()}")
                print(f"🎵 Tensor has NaN: {torch.isnan(mel_tensor).any()}, has Inf: {torch.isinf(mel_tensor).any()}")
                
                # Ensure tensor is contiguous and on the right device
                mel_tensor = mel_tensor.contiguous()
                if hasattr(self, 'device'):
                    mel_tensor = mel_tensor.to(self.device)
                
                print(f"🎵 Final tensor device: {mel_tensor.device}, is_contiguous: {mel_tensor.is_contiguous()}")
                
                return mel_tensor
                
            except Exception as e:
                print(f"❌ Binary audio extraction error: {e}")
                return torch.zeros((1, 32, 16), dtype=torch.float32)
        else:
            # Return default audio features (zeros) as fallback
            audio_tensor = torch.zeros((1, 32, 16), dtype=torch.float32)
            print(f"🔄 Using zero audio features as fallback: shape {audio_tensor.shape}")
            return audio_tensor
    ...
    def _generate_inference_only_sync(self, model_instance: ModelInstance, frame_id: int, audio_features: torch.Tensor) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # For real-time audio: audio_features has shape [1, 32, 16] (already processed mel spectrogram)
        # For package audio: audio_features has shape [num_frames, 512] (raw features per frame)
        if audio_features.dim() == 3 and audio_features.shape[0] == 1:
            # Real-time audio: use the processed mel spectrogram directly
            audio_slice = audio_features  # Shape: [1, 32, 16]
        else:
            # Package audio: slice by frame_id
            audio_slice = audio_features[frame_id:frame_id+1]  # Shape: (1, 512)
        ...
        # Handle audio tensor reshaping based on input format
        if audio_slice.shape[-1] == 16 and audio_slice.shape[-2] == 32:
            # Real-time audio: already in [1, 32, 16] format
            audio_reshaped = audio_slice.unsqueeze(-1).repeat(1, 1, 1, 16)  # (1, 32, 16, 16)
        else:
            # Package audio: reshape from [1, 512] to [1, 32, 16, 16]
            audio_tensor = audio_slice.unsqueeze(0)  # Shape: (1, 1, 512)
            audio_reshaped = audio_tensor.view(1, 32, 16).unsqueeze(-1).repeat(1, 1, 1, 16)  # (1, 32, 16, 16)
    ...
    def _calculate_audio_hash_binary(self, audio_binary: bytes) -> str:
        """🚀 OPTIMIZED: Calculate hash for raw binary audio data"""
        return hashlib.md5(audio_binary).hexdigest()
    ...
    def _extract_audio_sync(self, audio_data: str) -> torch.Tensor:
        """Synchronous audio extraction for real-time processing"""
        
        # For real-time audio, audio_data contains base64 encoded 16 chunks (640ms total)
        if audio_data and len(audio_data) > 100:  # Likely base64 data
            import base64
            from data_utils.ave import audio
            from data_utils.ave.hparams import hparams
            
            try:
                # Decode base64 audio data - this should be 16 chunks × 960 samples = 15,360 samples
                audio_bytes = base64.b64decode(audio_data)
                audio_array = np.frombuffer(audio_bytes, dtype=np.int16).astype(np.float32) / 32767.0
                
                print(f"🎵 Audio processing: {len(audio_array)} samples ({len(audio_array)/24000*1000:.1f}ms)")
                ...
                # Convert to tensor and reshape to match model expectation [1, 32, 16]
                # But we have [16, 80], so we need to understand the model's expectation
                mel_tensor = torch.from_numpy(cropped_mel).float()  # [16, 80]
                
                # For now, let's try reshaping. The model expects [1, 32, 16]
                # This might mean: [batch, mel_bins, time_frames] 
                # We have [time_frames, mel_bins] = [16, 80]
                # We need to transpose and take only 32 mel bins
                if cropped_mel.shape[1] >= 32:
                    # Take first 32 mel bins and transpose
                    mel_reshaped = cropped_mel[:, :32].T  # [32, 16]
                    mel_tensor = torch.from_numpy(mel_reshaped).unsqueeze(0).float()  # [1, 32, 16]
                ...
                print(f"🎵 Final tensor shape: {mel_tensor.shape}")
                print(f"🎵 Tensor min: {mel_tensor.min()}, max: {mel_tensor.max()}, mean: {mel_tensor.mean()}")
                print(f"🎵 Tensor has NaN: {torch.isnan(mel_tensor).any()}, has Inf: {torch.isinf(mel_tensor).any()}")
                ...
                import traceback
                traceback.print_exc()
                # Fallback to expected tensor shape
                return torch.zeros((1, 32, 16), dtype=torch.float32)
        
        # Fallback for file-based processing (shouldn't happen in real-time mode)
        return torch.zeros((1, 32, 16), dtype=torch.float32)
