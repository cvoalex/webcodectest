...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd D:\Projects\webcodecstest\go-inference-server; .\inference-server.exe" -WindowStyle Normal
Start-Sleep -Seconds 3

# Start compositing server in new window
Write-Host "Starting Compositing Server (CPU)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd D:\Projects\webcodecstest\go-compositing-server; .\compositing-server.exe" -WindowStyle Normal
Start-Sleep -Seconds 3
...
Set-Location D:\Projects\webcodecstest\go-compositing-server
.\test-client.exe
...
Write-Host "Close them manually when done." -ForegroundColor Yellow
Write-Host ""
